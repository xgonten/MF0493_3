...
export function EditarEstudios() {
  ...
  const { saber } = data || false
  useEffect(() => {
    getDocument('estudios', idDoc)
  ...
  const guardarEstudios = async data => {
    setDisabled(true)
    setIsloading(true)
    try {
      await updateDocument(idDoc, 'estudios', data)
      navigate('/estudios')
    } catch (error) {
        ...
        <div className="relative items-center justify-center w-1/2 p-2 px-6 m-auto mx-5 bg-slate-200 h-1/3 sm:h-1/3 md:w-1/3 md:h-1/3 lg:w-full lg: lg:h-1/3 rounded-3xl filter drop-shadow-2xl">
          ...
          <div className="mt-1 sm:mt-8">
            <form className="flex flex-col" onSubmit={handleSubmit}>
              <label htmlFor="saber" className="mb-3 text-base text-gray-700 sm:mt-4 sm:text-md">Estudios de:</label>
              <input name="saber" id="saber" type="text" defaultValue={saber} className="w-full h-4 border-b-2 border-gray-300 outline-none sm:h-9 focus:border-blue-300" />
