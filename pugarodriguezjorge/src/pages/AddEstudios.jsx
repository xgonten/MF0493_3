...
async function guardarDatos(data) {
  await addDocument('estudios', data) // prueba1 es la colección y data son los documentos(titulo y descripcion name en el html)
}

export function AddEstudios() {
        ...
        <div className="relative items-center justify-center w-1/2 p-2 px-6 m-auto mx-5 bg-white h-1/3 sm:h-1/3 md:w-1/3 md:h-1/3 lg:w-full lg: lg:h-1/3 rounded-3xl filter drop-shadow-2xl">
          <div className="mt-3 sm:mt-5">
            <h1 className="text-sm font-black tracking-wider text-gray-600 sm:text-md">
              Crea nuevos Estudios:
            </h1>
          </div>
          <div className="mt-1 sm:mt-8">
            <form className="flex flex-col" onSubmit={handleSubmit}>
              <label htmlFor="saber" className="mt-1 text-xs text-gray-700 sm:mt-5 sm:text-md">Estudios de:</label>
              <input name="saber" id="saber" type="text" className="w-full h-4 border-b-2 border-gray-300 outline-none sm:h-9 focus:border-blue-300" />
